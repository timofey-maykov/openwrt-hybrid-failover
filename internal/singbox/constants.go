...
const (
	...
	// EngineSocketMark is set (SO_MARK) on every socket the native engine
	// opens towards the outside. mangle_output returns on it first, so the
	// engine's own traffic is never re-marked into TPROXY. Without it a flow
	// the engine sends direct to an address in hf_proxy_subnets (for example
	// the Xbox Teredo server) came back into the engine as a new session on
	// every packet, thousands per second, until the router ran out of memory.
	EngineSocketMark     = 0x106
	EngineSocketMarkText = "0x106"
	FakeIPTestPort       = 8443
