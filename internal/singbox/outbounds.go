...
func (b *Builder) configureVPN(section string, sec *uci.Section) error {
	...
	var domainResolverTag string
	if sec.GetBool("domain_resolver_enabled", false) {
		ifaceTag := IfaceBindTag(section)
		b.cfg.AddOutbound(vpnDirectOutbound(ifaceTag, iface, ""))
		domainResolverTag = b.queueSectionDomainResolver(section, sec, ifaceTag)
	}
