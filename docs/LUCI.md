...
| **Обновление** | Проверка новой версии на GitHub и установка релиза прямо из LuCI |
...
Кнопка **Проверить обновления** спрашивает у GitHub последний релиз и показывает версию и список изменений. Если версия новее установленной, становится активной кнопка **Обновить**.

Обновление идёт в фоне: роутер скачивает пакеты своей архитектуры из релиза, сверяет каждый файл по `manifest.json` (sha256 и размер), ставит их через `apk` или `opkg`, делает `migrate` и перезапускает rpcd и службы. Страница сама следит за ходом, показывает журнал и перезагружается, когда всё готово. Настройки UCI не трогаются. Бот и curfew обновляются, только если уже стоят.
...
`apply --foreground` ставит в текущей консоли, `--tag v1.7.45` берёт конкретный релиз, `--force` разрешает ту же или более старую версию.
